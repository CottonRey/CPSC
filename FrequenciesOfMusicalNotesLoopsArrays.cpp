...
int main()
{
  ...
  for (int nu = 0; nu < k_nOctaves; nu++){
    for (int k = 0; k < k_nHalfTones; k++){
      ...
      fractionAmount = k / 12.0;
      calculatedFrequency = (referenceFrequency * pow(2, nu) * pow(2, fractionAmount));
      ...
      arrayNum = ((12 * nu) + k);
      strNote = a_strNotes[arrayNum];

      cout << "Note: " << strNote << "; nu: " << nu << "; k: " << k << "; frequency: " << calculatedFrequency << " Hz; wavelength: " << calculatedWavelength << " cm" << endl;
    }
  }
}
